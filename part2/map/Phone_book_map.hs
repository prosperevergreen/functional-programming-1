...
nameExists::Name -> Phonebook -> Bool 
nameExists = Map.member

entryExists::PhoneNo -> [Phone] -> Bool 
entryExists phoneNum phoneBookMap = not (null [ entry | entry <- phoneBookMap, phoneNo entry == phoneNum ])

findEntries :: Name -> Phonebook -> [Phone]
findEntries =  Map.findWithDefault []

addEntry :: Name -> String -> String -> String -> Phonebook -> Phonebook
addEntry nameStr phoneType countryCode phoneNo phoneBook
    | nameExists nameStr phoneBook = if entryExists phoneNum currentPhonebook then phoneBook else Map.adjust (readPhone phoneType countryCode phoneNo : ) nameStr phoneBook
    | otherwise = Map.insert nameStr [readPhone phoneType countryCode phoneNo] phoneBook
    where currentPhonebook = findEntries nameStr phoneBook
          phoneNum = toPhoneNoFromString phoneNo